...
export function NetstoneCaseStudyPage({ full = false }: { full?: boolean }) {
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      if (!full && event.key === "Escape") router.push("/", { scroll: false });
    };
  ...
  return (
        <div className={`case-overlay${full ? " is-full-page" : ""}`} role="presentation" onMouseDown={(event) => {
          if (!full && event.target === event.currentTarget) router.push("/", { scroll: false });
        }}>
          <section
          ...
          >
            ...
            {!full && <Link className="case-control case-close" href="/" scroll={false} aria-label="Close case study">
              <CloseIcon />
            </Link>}

            <div ref={scrollRef} className="case-scroll" onScroll={updateActiveChapter}>
              <header className="case-header">
                <Image src="/logo-claudio.svg" alt="Claudio Bakker" width={42} height={42} unoptimized />
